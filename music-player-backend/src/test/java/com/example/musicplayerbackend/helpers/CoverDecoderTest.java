...
class CoverDecoderTest {
  ...
  void shouldDecodePlainBase64() {
    String encoded =
        Base64.getEncoder().encodeToString("cover-bytes".getBytes(StandardCharsets.UTF_8));
  ...
  void shouldDecodeDataUrlBase64() {
    String encoded =
        Base64.getEncoder().encodeToString("jpeg-data".getBytes(StandardCharsets.UTF_8));
