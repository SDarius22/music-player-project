...
class ArtistControllerIntegrationTest extends BaseIntegrationTest {
  ...
  void shouldReturn200WithArtistResults() throws Exception {
    mockMvc
        .perform(get("/api/v1/artists").with(user(testUser)))
        ...
        .andExpect(
            jsonPath(
                "$.content[0].songFileHashes",
                containsInAnyOrder("artist-test-hash-001", "artist-test-hash-000")));
  }

  @Test
  void shouldFilterArtistsByQuery() throws Exception {
    mockMvc
        .perform(get("/api/v1/artists").param("q", "Test").with(user(testUser)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content", not(empty())));
  }
  ...
  void shouldReturnEmptyArtistsWhenQueryMatchesNothing() throws Exception {
    mockMvc
        .perform(get("/api/v1/artists").param("q", "zzznomatch").with(user(testUser)))
        .andExpect(status().isOk())
  ...
  void shouldReturn200ForArtistByHash() throws Exception {
    mockMvc
        .perform(get("/api/v1/artists/{artistHash}", artist.getHash()).with(user(testUser)))
        .andExpect(status().isOk())
  ...
  void shouldReturn404WhenArtistNotFound() throws Exception {
    mockMvc
        .perform(get("/api/v1/artists/999999").with(user(testUser)))
  ...
  void shouldReturn200ForArtistCover() throws Exception {
    mockMvc
        .perform(get("/api/v1/artists/{artistHash}/cover", artist.getHash()).with(user(testUser)))
        .andExpect(status().isOk())
  ...
  void shouldReturn404WhenArtistCoverNotFound() throws Exception {
    mockMvc
        .perform(get("/api/v1/artists/999999/cover").with(user(testUser)))
        .andExpect(status().isNotFound());
  }
  ...
  void shouldReturnArtistSongsOrderedByName() throws Exception {
    mockMvc
        .perform(get("/api/v1/artists/{artistHash}/songs", artist.getHash()).with(user(testUser)))
        .andExpect(status().isOk())
