...
class SongControllerIntegrationTest extends BaseIntegrationTest {
  ...
  void setUp() {
    ...
    Album album =
        albumRepository.save(Album.builder().name("Test Album").artists(Set.of(artist)).build());
  ...
  void shouldReturn200WithPublicSongs() throws Exception {
    mockMvc
        .perform(get("/api/v1/songs").with(user(testUser)))
  ...
  void shouldFilterSongsByNameQuery() throws Exception {
    mockMvc
        .perform(get("/api/v1/songs").param("q", "Public").with(user(testUser)))
        .andExpect(status().isOk())
        .andExpect(
            jsonPath(
                "$.content[?(@.fileHash == '" + publicSong.getFileHash() + "')].name",
  ...
  void shouldReturnEmptySongPageWhenQueryMatchesNothing() throws Exception {
    mockMvc
        .perform(get("/api/v1/songs").param("q", "zzznomatch").with(user(testUser)))
        .andExpect(status().isOk())
  ...
  void shouldIncludePrivateSongsOwnedByCurrentUser() throws Exception {
    mockMvc
        .perform(get("/api/v1/songs").param("q", "Private").with(user(testUser)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content", not(empty())));
  }
  ...
  void shouldRespectPaginationWhenFetchingSongs() throws Exception {
    mockMvc
        .perform(get("/api/v1/songs").param("size", "1").with(user(testUser)))
        .andExpect(status().isOk())
  ...
  void shouldSortSongsByYear() throws Exception {
    mockMvc
        .perform(get("/api/v1/songs").param("sort", "year").with(user(testUser)))
        .andExpect(status().isOk())
  ...
  void shouldSortSongsByYearDesc() throws Exception {
    mockMvc
        .perform(get("/api/v1/songs").param("sort", "year,desc").with(user(testUser)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content").isArray());
  }
  ...
  void shouldSortSongsByDurationInSeconds() throws Exception {
    mockMvc
        .perform(get("/api/v1/songs").param("sort", "durationInSeconds").with(user(testUser)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content").isArray());
  }
  ...
  void shouldSortSongsByTrackNumber() throws Exception {
    mockMvc
        .perform(get("/api/v1/songs").param("sort", "trackNumber").with(user(testUser)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content").isArray());
  }
  ...
  void shouldSortSongsByDiscNumber() throws Exception {
    mockMvc
        .perform(get("/api/v1/songs").param("sort", "discNumber").with(user(testUser)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content").isArray());
  }
  ...
  void shouldSortSongsByNameExplicitAsc() throws Exception {
    mockMvc
        .perform(get("/api/v1/songs").param("sort", "name,asc").with(user(testUser)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content").isArray());
  }
  ...
  void shouldUseDefaultSortWhenUnknownSortPropertyProvided() throws Exception {
    mockMvc
        .perform(get("/api/v1/songs").param("sort", "unknown").with(user(testUser)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content").isArray());
  }
  ...
  void shouldReturn200ForSongById() throws Exception {
    mockMvc
        .perform(get("/api/v1/songs/{fileHash}", publicSong.getFileHash()).with(user(testUser)))
        .andExpect(status().isOk())
  ...
  @Test
  void shouldReturn404WhenSongByIdNotFound() throws Exception {
    mockMvc
        .perform(get("/api/v1/songs/999999").with(user(testUser)))
  ...
  void shouldReturn200WhenSongCoverAlbumHasCover() throws Exception {
    mockMvc
        .perform(
            get("/api/v1/songs/{fileHash}/cover", songWithCover.getFileHash()).with(user(testUser)))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.IMAGE_JPEG));
  }

  @Test
  void shouldReturn404WhenSongCoverIsMissing() throws Exception {
    mockMvc
        .perform(
            get("/api/v1/songs/{fileHash}/cover", songWithNoAlbum.getFileHash())
                .with(user(testUser)))
        .andExpect(status().isNotFound());
  }
  ...
  void shouldReturn200ForRecommendations() throws Exception {
    mockMvc
        .perform(get("/api/v1/songs/recommendations").with(user(testUser)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content").isArray());
  }
  ...
  void shouldReturn200ForForgottenFavourites() throws Exception {
    mockMvc
        .perform(get("/api/v1/songs/forgotten").with(user(testUser)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content").isArray());
  }
  ...
  void shouldReturn200ForQuickDial() throws Exception {
    mockMvc
        .perform(get("/api/v1/songs/quick-dial").with(user(testUser)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content").isArray());
  }
  ...
  void shouldReturn400WhenChunkHashMismatch() throws Exception {
    ...
    mockMvc
        .perform(
            multipart("/api/v1/songs/{fileHash}/chunks/{index}", privateSong.getFileHash(), 0)
                .file(chunkFile)
  ...
  void shouldReturn403WhenNonAdminUserUploadsSong() throws Exception {
    MockMultipartFile file =
        new MockMultipartFile("file", "song.mp3", "audio/mpeg", "fake-audio".getBytes());

    mockMvc
        .perform(
            multipart("/api/v1/songs")
                .file(file)
