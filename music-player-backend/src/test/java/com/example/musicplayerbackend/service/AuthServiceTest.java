...
class AuthServiceTest {
  ...
  @Mock CodeMapper codeMapper;
  @Mock JavaMailSender mailSender;
  @Mock DefaultPlaylistService defaultPlaylistService;

  @Captor ArgumentCaptor<VerificationCode> vcCaptor;
  @Captor ArgumentCaptor<MimeMessage> mailCaptor;
  ...
  void shouldUpdateExistingVerificationCode() {
    ...
    verify(codeRepository).save(vcCaptor.capture());
    assertSame(existing, vcCaptor.getValue());
    assertNotEquals("123456", vcCaptor.getValue().getCode());
  ...
  void shouldReturnNewAccessTokenWhenRefreshTokenIsValid() {
    User user =
        User.builder().id(1L).email("a@b.com").role(Role.USER).provider(AuthProvider.LOCAL).build();
