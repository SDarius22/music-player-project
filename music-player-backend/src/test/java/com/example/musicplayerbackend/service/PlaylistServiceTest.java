...
class PlaylistServiceTest {
  ...
  void shouldReturnPagedPlaylistDtos() {
    ...
    dto.setName("My Mix");
    when(playlistRepository.findAllWithHashes(eq(1L), any(), any(), any()))
  ...
  void shouldPersistPlaylistSongsInRequestedOrderWhenCreatingPlaylist() {
    ...
    Song song1 =
        Song.builder().id(10L).name("S1").songType(ContentType.STREAMABLE).fileHash("h1").build();
    Song song2 =
        Song.builder().id(20L).name("S2").songType(ContentType.STREAMABLE).fileHash("h2").build();
    ...
    PlaylistSong ps0 =
        PlaylistSong.builder().id(new PlaylistSongId(99L, 0)).playlist(saved).song(song2).build();
    PlaylistSong ps1 =
        PlaylistSong.builder().id(new PlaylistSongId(99L, 1)).playlist(saved).song(song1).build();
  ...
  void shouldReturnSongsInPositionOrderWhenGettingPlaylistDetail() {
    Playlist playlist =
        Playlist.builder()
            ...
            .build();
    Song song1 =
        Song.builder().id(11L).fileHash("h1").songType(ContentType.STREAMABLE).name("S1").build();
    Song song2 =
        Song.builder().id(22L).fileHash("h2").songType(ContentType.STREAMABLE).name("S2").build();

    PlaylistSong ps0 =
        PlaylistSong.builder().id(new PlaylistSongId(5L, 0)).playlist(playlist).song(song2).build();
    PlaylistSong ps1 =
        PlaylistSong.builder().id(new PlaylistSongId(5L, 1)).playlist(playlist).song(song1).build();

    when(playlistRepository.findById(5L)).thenReturn(Optional.of(playlist));
    when(playlistSongRepository.findByPlaylist_IdOrderById_PositionAsc(5L))
        .thenReturn(List.of(ps0, ps1));
  ...
  void shouldReplaceSongsWhenUpdateSongHashesProvided() {
    ...
    Song song =
        Song.builder()
            ...
            .fileHash("h")
            .build();
    PlaylistSong relation =
        PlaylistSong.builder().id(new PlaylistSongId(1L, 0)).playlist(playlist).song(song).build();
    when(playlistRepository.findById(1L)).thenReturn(Optional.of(playlist));
    when(playlistRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));
  ...
  void shouldThrow400WhenUpdateContainsDuplicatePositions() {
    ...
    ResponseStatusException ex =
        assertThrows(ResponseStatusException.class, () -> service.updatePlaylist(1L, 1L, req));
    assertEquals(HttpStatus.BAD_REQUEST, ex.getStatusCode());
  }
  ...
  void shouldThrow404WhenPlaylistByIdNotFound() {
    ...
    ResponseStatusException ex =
        assertThrows(ResponseStatusException.class, () -> service.getPlaylistById(99L, 1L));
  ...
  void shouldThrow403WhenPlaylistOwnedByDifferentUser() {
    ...
    ResponseStatusException ex =
        assertThrows(ResponseStatusException.class, () -> service.getPlaylistById(1L, 999L));
  ...
  void shouldThrow403WhenGettingPlaylistCoverOwnedByDifferentUser() {
    ...
    when(playlistRepository.findById(1L)).thenReturn(Optional.of(p));
    ResponseStatusException ex =
        assertThrows(ResponseStatusException.class, () -> service.getPlaylistCover(1L, 999L));
    assertEquals(HttpStatus.FORBIDDEN, ex.getStatusCode());
  }
}
