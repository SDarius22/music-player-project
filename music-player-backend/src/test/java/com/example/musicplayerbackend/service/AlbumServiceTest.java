...
class AlbumServiceTest {
  ...
  void setUp() {
    ...
    service =
        new AlbumService(
            albumRepository, songRepository, albumMapper, sortMapper, songEnrichmentService);
  ...
  void shouldThrowNotFoundWhenDecodeCoverImageIsNull() {
    ResponseStatusException ex =
        assertThrows(ResponseStatusException.class, () -> CoverDecoder.decodeCoverImage(null));
  ...
  void shouldDecodeRawBase64CoverImage() {
    ...
    assertArrayEquals(
        expected, CoverDecoder.decodeCoverImage(Base64.getEncoder().encodeToString(expected)));
  }

  @Test
  void shouldThrowNotFoundWhenCoverImageBase64IsInvalid() {
    assertThrows(
        ResponseStatusException.class, () -> CoverDecoder.decodeCoverImage("!!!invalid!!!"));
  }
  ...
  void shouldSortAlbumsAscendingWhenExplicitAscProvided() {
    ...
    when(albumRepository.findAllByNameContainingIgnoreCase(
            any(),
            argThat(
                p ->
  ...
  void shouldSplitSongHashesFromAlbumSongs() {
    ...
    Album album =
        Album.builder()
            ...
            .name("Album Name")
            .artists(Set.of(artist))
            .songs(List.of(song1, song2, song3))
            .build();
    ...
    assertEquals(
        List.of("hash1", "hash2", "hash3"), result.getContent().getFirst().getSongFileHashes());
  }
  ...
  void shouldPickArtistWithMostSongsAsMainArtist() {
    ...
    Artist featuredArtist =
        Artist.builder().id(2L).hash("featured-hash").name("Featured Artist").build();
    Album album =
        Album.builder()
            .hash("album-hash")
            .name("Album Name")
  ...
  void shouldReturnAlbumDetailDto() {
    Album album =
        Album.builder().id(1L).hash("thriller-hash").name("Thriller").songs(List.of()).build();
  ...
  void shouldThrow404WhenAlbumByHashNotFound() {
    ...
    ResponseStatusException ex =
        assertThrows(
            ResponseStatusException.class, () -> service.getAlbumByHash("missing-hash", 1L));
    assertEquals(HttpStatus.NOT_FOUND, ex.getStatusCode());
  }
  ...
  void shouldMapSongsWhenGettingAlbumById() {
    Song song =
        Song.builder()
            ...
            .build();
    Album album =
        Album.builder().id(1L).hash("thriller-hash").name("Thriller").songs(List.of(song)).build();
    when(albumRepository.findByHash("thriller-hash")).thenReturn(Optional.of(album));
  ...
  void shouldReturnEmptySongsWhenAlbumSongsAreNull() {
    Album album =
        Album.builder().id(1L).hash("no-songs-hash").name("No Songs").songs(List.of()).build();
  ...
  void shouldThrow404WhenAlbumCoverIsNull() {
    ...
    ResponseStatusException ex =
        assertThrows(ResponseStatusException.class, () -> service.getAlbumCover("null-cover-hash"));
    assertEquals(HttpStatus.NOT_FOUND, ex.getStatusCode());
  }
}
