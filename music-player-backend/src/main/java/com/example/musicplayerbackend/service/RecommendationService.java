...
public class RecommendationService {
    ...
    public Page<SongDto> getForgottenFavourites(Long userId, Pageable pageable) {
        ...
        return withRandomPadding(
                userLibraryRepository.findForgottenByUserId(userId, cutoff, pageable),
                userId, pageable);
    }
    ...
    public Page<SongDto> getQuickDial(Long userId, Pageable pageable) {
        return withRandomPadding(
                userLibraryRepository.findQuickDialByUserId(userId, pageable),
                userId, pageable);
    }
    ...
    private Page<SongDto> withRandomPadding(Page<UserLibrary> matched, Long userId, Pageable pageable) {
        ...
        if (deficit > 0) {
            List<Song> randomSongs = songRepository.findRandomStreamable(
                    PageRequest.of(0, deficit * 3));
            if (!randomSongs.isEmpty()) {
                List<Long> randomIds = randomSongs.stream().map(Song::getId).toList();
                Map<Long, UserLibrary> entriesById = userLibraryRepository
                        .findByIdUserIdAndIdSongIdIn(userId, randomIds).stream()
