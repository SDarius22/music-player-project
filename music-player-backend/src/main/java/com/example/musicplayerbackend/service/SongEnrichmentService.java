...
public class SongEnrichmentService {
  ...
  public List<SongDto> enrich(List<Song> songs, Long userId) {
    ...
    List<Long> songIds =
        songs.stream().map(Song::getId).filter(java.util.Objects::nonNull).toList();
    Map<Long, UserLibrary> byId =
        userLibraryRepository.findByIdUserIdAndIdSongIdIn(userId, songIds).stream()
            .filter(ul -> !Boolean.TRUE.equals(ul.getIsDeleted()))
            .collect(Collectors.toMap(ul -> ul.getId().getSongId(), Function.identity()));
    return songs.stream().map(s -> songMapper.toDto(s, byId.get(s.getId()))).toList();
  }
}
