...
public class PlaylistService {
  ...
  public PlaylistPageDto getPlaylists(Long userId, String q, String sort,
      Boolean indestructibleFilter, Boolean includeQueue, int page, int size) {
    ...
    if (Boolean.TRUE.equals(includeQueue) && page == 0) {
      ...
      playlistRepository.findAllWithHashes(userId, DefaultPlaylistService.QUEUE, null,
              PageRequest.of(0, 50))
          .getContent().stream()
  ...
  public byte[] getPlaylistCover(Long playlistId, Long userId) {
    ...
    List<PlaylistSong> playlistSongs = playlistSongRepository.findByPlaylist_IdOrderById_PositionAsc(
        playlist.getId());
  ...
  private void replacePlaylistSongs(Playlist playlist, List<PlaylistSongPositionDto> items) {
    ...
    for (PlaylistSongPositionDto item : items) {
      if (item == null || item.getSongFileHash() == null || item.getSongFileHash().isBlank()
      ...
      if (!usedPositions.add(item.getPosition())) {
        throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
            "Duplicate playlist position: " + item.getPosition());
      }
    }
