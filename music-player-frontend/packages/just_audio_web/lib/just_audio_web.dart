...
class Html5AudioPlayer extends JustAudioPlayer {
  ...
  Html5AudioPlayer({required String id}) : super(id: id) {
    ...
    // Vendored change: unblock load() as soon as metadata is available instead
    // of waiting only for `durationchange`. For some formats served over the
    // P2P service-worker stream the browser emits `durationchange` only after
    // it has scanned (nearly) the whole file, which delayed `setAudioSource()`
    // — and therefore playback start — until the entire song had downloaded.
    // `loadedmetadata` fires as soon as the header/duration is known, letting
    // playback begin while the rest of the file streams progressively.
    _audioElement.addEventListener(
        'loadedmetadata',
        (Event event) {
          _durationCompleter?.complete();
