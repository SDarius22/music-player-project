...
class SongsRepository {
  ...
  Stream watchAllSongs({bool triggerImmediately = true}) {
    final query = songBox._query();
    return query.watch();
  }

  Song? getSong(String path)  {
    return songBox._query(Song_.path.equals(path)).build().findUnique();
  }
  ...
  Song? getSongContaining(String query)  {
    ...
    return songBox._query(Song_.path.contains(query, caseSensitive: false)).build().findFirst();
  }
  ...
  List<Song> getSongs(String query, String sortField, bool flag)  {
    ...
    if (flag == false) {
      builderQuery = songBox
          ._query(Song_.name.contains(query, caseSensitive: false))
          .order(
  ...
  List<Song> getAllSongs()  {
    return songBox._query().order(Song_.name).build().find();
  }
  ...
  List<Song> getFavoriteSongs()  {
    return songBox._query(Song_.liked.equals(true)).order(Song_.name).build().find();
  }

  List<Song> getSongsWithLastPlayed()  {
    Query<Song> query = songBox._query(Song_.lastPlayed.notNull()).order(Song_.lastPlayed, flags: Order.descending).build();
    query.limit = 100;
    return query.find();
  }

  List<Song> getSongsWithPlayCount()  {
    Query<Song> query = songBox._query(Song_.playCount.greaterThan(0)).order(Song_.playCount, flags: Order.descending).build();
    query.limit = 100;
    return query.find();
  }
}