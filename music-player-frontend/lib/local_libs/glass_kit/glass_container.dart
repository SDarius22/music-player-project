...
class GlassContainer extends StatelessWidget {
  ...
  const GlassContainer({
    ...
    this.shape = BoxShape.rectangle,
       ...
       borderRadius =
           shape == BoxShape.rectangle ? (borderRadius ?? kBorderRadius) : null,
       assert(
       ...
       assert(
         ...
         'borderColor cannot be null when running on the Web\n',
       );
  ...
  /// to cast as the shadow. The [height] is used as the diameter of the circle
  ///
  ...
  Widget build(BuildContext context) {
    ...
    // In case the app is compiled to run on web then CustomPaint wont work
    if (!_colorOnlyBorder && !kIsWeb) {
    ...
    // Combine the backdropFilter, frosted layer, and container into a stack
    current = Stack(
    ...
    if (_isCircle) {
      assert(borderRadius == null);
      current = ClipOval(clipper: CircleClipper(), child: current);
    } else {
      assert(borderRadius != null);
      current = ClipRRect(
...
class _FrostedWidget extends StatelessWidget {
  /// Creates a Frosted Layer Widget
  const _FrostedWidget({this.width, this.height, required this.frostedOpacity});
