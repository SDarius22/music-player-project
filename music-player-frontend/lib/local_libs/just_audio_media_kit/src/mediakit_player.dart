...
import '../just_audio_media_kit.dart';
...
class MediaKitPlayer extends AudioPlayerPlatform {
  ...
  MediaKitPlayer(super.id) {
    ...
    setProperty(_player, 'auto-subs', 'no');
    setProperty(_player, 'autoload-files', 'no');
  ...
  void _fixIndecies({int? current, int? shuffled}) {
    assert(
  ...
  Future<void> _sendAudios() async {
    ...
    final int maxSize = (_currentIndex + JustAudioMediaKit.prefetchPlaylistSize)
      ...
      Playlist(
        _isShuffling
            ? _shuffleOrder
                ...
                .map((index) => _playlist![index])
                .toList()
            : _playlist!.sublist(_currentIndex, maxSize),
      ),
      play: _playing,
    );
  }
  ...
  Future<LoadResponse> load(LoadRequest request) async {
    ...
    if (request.audioSourceMessage is ConcatenatingAudioSourceMessage) {
      ...
      final playable = _convertAudioSourceToMediaKit(
